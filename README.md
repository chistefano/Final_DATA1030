...
In this project, I used six different machine-learning models to predict future stock prices. Among them, the Ridge Regression model emerged as the most accurate for forecasting stock prices, achieving the lowest average Root Mean Squared Error (RMSE) of 2.35 dollars. This figure suggests that the model's predictions typically deviate by about 2.35 dollars from the actual stock prices. The predictions made by the Ridge Regression Mdoel with tuned hyperparameters is saved in the 'Data' repository  under 'y_pred_ridge'. 
